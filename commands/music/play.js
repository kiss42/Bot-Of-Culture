...
module.exports = class PlayCommand extends Command {
    constructor(client) {
        super(client, {
            name: 'play',
            memberName: 'play',
            group: 'music', // this is referring to the command group
            description: 'Play any song or playlist from youtube',
            guildOnly: true,  //only available in servers, not DMs
            clientPermissions: ['SPEAK', 'CONNECT'],
            args: [
                {
                    key: 'query', // The variable that will hold the input
    ...
    /**
     * 
     * @param {*} message The provided messaged representing the command
     * @param {*} param1  The command arguments signifying the query
        ...
        */
        if (query.match(/^(?!.*\?.*\bv=)https:\/\/www\.youtube\.com\/.*\?.*\blist=.*$/)){
            try {
                ...
                const videosObjs = await playlist.getVideos();     // songs data object
                //const videos = Object. entries(videosObjs);      // turn the object to array
                //iterate through the videos array and make a song object out of each vid
                for (let i = 0; i < videosObjs.length; i++){
                    const video = await videosObj[i].fetch();

                    const url = `https://www.youtube.com/watch?v=${video.raw.id}`;
                    const title = video.raw.snippet.title;
                    let duration = this.formationDuration(video.duration);
                    const thumbnail = video.thumbnails.high.url;
                    if (duration =='00:00') duration = 'Live Stream';
                    const song = { url, title, duration, thumbnail, voiceChannel };
                    
                    message.guild.musicData.queue.push(song); // Push to the queue of this server
                }
                if (message.guild.musicData.isPlaying == false) {
                    ...
                    return message.say(`Playlist - :musical_note: ${playlist.title} : musical_note: has been added to queue`);
                }
            }
            catch (error) {
                console.error(error);
    ...
    /**
     * Tells the bot to join the user's voice channel and begin playing music from the queue
     * @param {*} queue         Object representing the current queue and associated songs
     * @param {String} message  Associated message/command of this queue
     */
    ...
    /**
     * Formats the given object into a youtube-style duration
     * @param {*} durationObj Duration time to be formatted
     */
